...
const Player: React.FC<PlayerProps> = ({
  ...
  // --- helpers must come before inner components ---
  ...
  const handleSkip = useCallback(
    (seconds: number) => {
      ...
      const newTime = Math.max(
        0,
        Math.min(podcast.duration || 0, audioRef.current.currentTime + seconds)
      );
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center gap-8 w-full">
        <div className="relative" style={{ width: size, height: size }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <h3 className="text-4xl font-bold text-brand-text mb-2">
              {formatTime((podcast.duration || 0) - currentTime)}
            </h3>
            <button
        ...
        <div className="flex items-center justify-center gap-2 w-full max-w-sm">
          <button
            ...
            className="text-brand-text-secondary hover:text-brand-text p-4 rounded-full text-sm transform transition-transform hover:scale-110 active:scale-95"
          >
            <RedoIcon size={24} className="backward -scale-x-100" />
          </button>
          <button
            onClick={handleCycleSpeed}
            className="text-brand-text font-semibold p-3 rounded-md w-24 text-center bg-brand-surface hover:bg-brand-surface-light transition-colors b-border transform hover:scale-105 active:scale-95"
          >
            {playbackRate}x
          </button>
          <button
  ...
  const DefaultExpandedPlayer = () => {
    ...
    const sharedControls = (
      <div className="flex items-center justify-center gap-2 w-full max-w-sm">
        ...
        <button
          ...
          className="text-brand-text-secondary hover:text-brand-text p-2 rounded-full text-sm transform transition-transform hover:scale-110 active:scale-95"
        >
          -5s
        </button>
        <button
          ...
          className="bg-brand-primary text-brand-text-on-primary rounded-full p-5 hover:bg-brand-primary-hover transition-transform transform active:scale-95 b-border b-shadow"
        >
          {isPlaying ? <PauseIcon size={32} /> : <PlayIcon size={32} />}
        </button>
        <button
    ...
    return (
      <div className="flex-grow flex flex-col items-center justify-center text-center gap-6 sm:gap-8">
        <div
          className={`w-56 h-56 sm:w-64 sm:h-64 md:w-80 md:h-80 bg-brand-surface rounded-lg shadow-2xl overflow-hidden b-border b-shadow transition-transform ${
            isPlaying ? 'animate-pulse-slow' : ''
          }`}
        >
          <img
            src={artworkUrl || 'https://i.imgur.com/Q3QfWqV.png'}
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed left-0 right-0 z-20 transition-all duration-500 ease-in-out ${
          isPlayerExpanded ? 'bottom-0 top-0' : 'bottom-0'
        }`}
      >
        <div
          className={`absolute inset-0 flex flex-col p-4 sm:p-8 transition-transform duration-300 ease-in-out ${
            isPlayerExpanded ? 'translate-y-0' : 'translate-y-full pointer-events-none'
          }`}
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 bg-brand-surface-light shadow-2xl transition-transform duration-500 ease-in-out cursor-pointer b-border ${
          ...
          <div className="max-w-4xl mx-auto flex items-center gap-3 p-2 sm:p-3">
            ...
            <div className="flex-grow min-w-0">
              ...
              <p className="text-xs text-brand-text-secondary">
                {formatTime(currentTime)} / {formatTime(podcast.duration || 0)}
              </p>
            </div>
            <div className="flex-shrink-0 flex items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className="bg-brand-primary text-brand-text-on-primary rounded-full p-2 hover:bg-brand-primary-hover transition-transform transform active:scale-95 b-border b-shadow"
              >
